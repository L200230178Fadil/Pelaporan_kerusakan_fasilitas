...
function RoleCard({ role, icon: Icon, label, desc, selected, onClick }) {
  return (
    ...
    >
      ...
      <div>
        <p className={`font-medium text-sm ${selected ? 'text-brand-700 dark:text-brand-300' : 'text-ink-800 dark:text-ink-200'}`}>{label}</p>
...
export default function FormLaporanPage() {
  ...
  return (
    <div className="max-w-xl mx-auto space-y-7">
      {/* Header */}
      <div data-anim="1">
        <Link to="/" className="inline-flex items-center gap-1.5 text-sm text-ink-500 hover:text-ink-800 mb-4 transition-colors">
          <ChevronLeft size={16} /> Kembali ke Dashboard
        </Link>
        <h1 className="font-display font-bold text-2xl text-ink-900">Lapor Kerusakan</h1>
        <p className="text-ink-500 text-sm mt-1">Isi form di bawah ini untuk melaporkan kerusakan fasilitas di Gedung J.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div data-anim="2" className="card p-5 space-y-4">
            ...
            <FieldGroup label="Foto Kerusakan (Opsional)">
              {fotoURL ? (
                ...
                <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-ink-200 rounded-2xl p-6 cursor-pointer hover:border-brand-400 hover:bg-brand-50/50 dark:hover:bg-brand-900/10 transition-all group">
                  <div className="w-10 h-10 rounded-xl bg-ink-100 group-hover:bg-brand-100 dark:group-hover:bg-brand-900/30 flex items-center justify-center transition-colors">
        ...
        {role && (
          <div data-anim="3" className="space-y-3">
            <div className="flex items-start gap-2.5 p-3.5 rounded-xl bg-amber-50 border border-amber-100 dark:bg-amber-950/30 dark:border-amber-900/50">
            ...
            <button type="submit" disabled={submitting} className="btn-primary w-full py-3 text-base font-bold bg-accent-500 text-ink-900 hover:bg-accent-600 border-none shadow-sm transition-transform hover:-translate-y-0.5">
              {submitting ? (
